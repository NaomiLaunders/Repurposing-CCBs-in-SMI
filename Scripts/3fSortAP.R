...
PatAPAll$PRN<-case_when(grepl("PRN|REQ|NECESSARY|MDU|UNKNOWN|NEEDED|DO NOT PRESCRIBE|	
DO NOT DISPENSE", PatAPAll$dosage_text, ignore.case=TRUE) ~1,
                        TRUE ~0)
table(PatAPAll$PRN)
...
DD$Quantity[DD$dosage_text=="56"]<-56
...
DD$dose<-case_when(DD$dosage_text=="1 THREE A DAY" ~ 3,
                   ...
                   DD$dosage_text=="12" ~ 12,
                         ...
                         DD$dosage_text=="5ML A DAY"|DD$dosage_text=="50D"|DD$dosage_text=="FIVE"|DD$dosage_text=="THREE 5ML SPOONSFUL TO BE TAKEN" ~ 5,
                   DD$dosage_text=="1B" ~ 2,
                   DD$dosage_text=="1T" ~ 3,
                   DD$dosage_text=="1 DB" ~ 2,
  ...
  DD$dosage_text=="20MG" ~ 20,
  DD$dosage_text=="2.5 MLS" ~ 2.5,
  DD$dosage_text=="0.5ML BEFORE FEEDS" ~ 0.5,
  ...
  DD$dosage_text=="HALF TABLET"|DD$dosage_text=="HALF TAB DA"|DD$dosage_text=="ONE HALF A DAY"|DD$dosage_text=="TAKE HALF A TABLET" ~ 0.5,
  ...
  DD$dosage_text=="NIGHTLY"| DD$dosage_text=="DAY"|DD$dosage_text=="O"|	
    DD$dosage_text=="0NE A DAY"|DD$dosage_text=="IDAILY"|DD$dosage_text=="DAILT"~ 1,
...
DD$dose[DD$dosage_text=="ID"]<-1
DD$dose[DD$dosage_text=="TWICE WEEK"]<-0.3
DD$dose[DD$dosage_text=="0.1"]<-0.1
DD$dose[DD$dosage_text=="I D"]<-1
DD$dose[DD$dosage_text=="ALTERNATIVE DAYS"]<-0.5
DD$dose[DD$dosage_text=="1 ALTERNATIVE DAYS"]<-0.5
...
#Can we get missing strength from text? We can get them all from product name for these prescriptions
...
Check<-select(PatAPAll, strength, StrengthNew)
...
save(Dur, file=paste0(ProjectDir, "/Extracts/AP_interim_CCB.Rdata"))
...
save(apDoseFinal, file=paste0(ProjectDir, "/Extracts/AP_dose_CCB.Rdata"))
save(apCheck, file=paste0(ProjectDir, "/Extracts/Missing_AP_dose_CCB.Rdata"))
...
save(BPDoseFinal, file=paste0(ProjectDir, "/Extracts/BP_dose_CCB.Rdata"))

save(BPCheck, file=paste0(ProjectDir, "/Extracts/MissingBP_dose_CCB.Rdata"))
