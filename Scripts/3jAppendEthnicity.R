...
EthnGroup1<-EthnGroup1%>%
  mutate(Group=case_when(desc=="other ethnic non-mixed (nmo)"|desc=="n african arab/iranian (nmo)"~ "Other",
                         ...
                         grepl("australian", desc, ignore.case=TRUE) ~ "Other",
                         TRUE ~ "Unknown"))
...
EthnGroup2<-EthnGroup2%>%
  mutate(Group=case_when(grepl("not|unknown", desc, ignore.case=TRUE) ~ "Unknown",
                         ...
                         grepl("arabic|pushto|persian|abkhazian|turkish", desc, ignore.case=TRUE) ~ "Other",
                         grepl("russian|gaelic|german", desc, ignore.case=TRUE) ~ "White",
                         TRUE ~ "DROP"))
...
save(Trial, file=paste0(ProjectDir, "/Data/Trial_ethn.rdata"))
