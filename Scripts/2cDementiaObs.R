...
AurumDementia<-ReadAurumMedCodelist(paste0(OldCodeListDir, "/Dementia22/DementiaAurum.txt"))
GoldDementia<-ReadGoldMedCodelist(paste0(OldCodeListDir, "/Dementia22/DementiaGold.txt"))

####Select only observations that are Dementia - GOLD####
...
#Select those that are dementia 
...
for (i in 1:(length(Observation_files))) {
  ...
  PatObs<-merge(x=GoldDementia, y=PatObs, by ="medcode", all.y = FALSE, all.x=FALSE)
...
save(PatObsGoldClin, file=paste0(ProjectDir, "/Extracts/DementiaGoldClinical.Rdata"))
...
save(PatObsGoldTest, file=paste0(ProjectDir, "/Extracts/DementiaGoldTest.Rdata"))
...
AurumDementia<-rename(AurumDementia, medcodeid = MedCodeId)
...
for (i in 1:(length(Observation_files))) {
  ...
  PatObs<-merge(x=AurumDementia, y=PatObs, by ="medcodeid", all.y = FALSE, all.x=FALSE)
...
PatObsAurum<-select(PatObsAurum, medcode=medcodeid, term, patid, eventdate=obsdate, sysdate=enterdate, value, numunitid)
...
save(PatObsAurum, file=paste0(ProjectDir, "/Extracts/DementiaAurum.Rdata"))

####Merge Aurum and Gold files 
PatObsGoldClin$patid<-paste0(PatObsGoldClin$patid, "-G")
length(which(PatObsGoldTest$patid %in% PatObsGoldClin$Test))

PatObsGoldClin<-select(PatObsGoldClin, -enttype, -adid, -patdate, -Group, -Umbrella)
PatObsAurum<-select(PatObsAurum, -value, -numunitid)

PatDementiaAll<-rbind(PatObsAurum, PatObsGoldClin)

save(PatDementiaAll, file=paste0(ProjectDir, "/Extracts/DementiaAll.Rdata"))
