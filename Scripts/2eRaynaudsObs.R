...
AurumRay<-ReadAurumMedCodelist("Codelists/RayAurum.txt")
GoldRay<-ReadGoldMedCodelist("Codelists/RayGold.txt")

####Select only observations that are Ray - GOLD####
...
for (i in 1:(length(Observation_files))) {
  ...
  PatObs<-merge(x=GoldRay, y=PatObs, by ="medcode", all.y = FALSE, all.x=FALSE)
...
save(PatObsGoldClin, file=paste0(ProjectDir, "/Extracts/RayGoldClinical.Rdata"))
...
AurumRay<-rename(AurumRay, medcodeid = MedCodeId)
...
for (i in 1:(length(Observation_files))) {
  ...
  PatObs<-merge(x=AurumRay, y=PatObs, by ="medcodeid", all.y = FALSE, all.x=FALSE)
...
PatObsAurum<-select(PatObsAurum, medcode=medcodeid, term = Term, patid, eventdate=obsdate, sysdate=enterdate, value, numunitid)
...
save(PatObsAurum, file=paste0(ProjectDir, "/Extracts/RayAurum.Rdata"))

####Merge Aurum and Gold files 
PatObsGoldClin$patid<-paste0(PatObsGoldClin$patid, "-G")
length(which(PatObsGoldTest$patid %in% PatObsGoldClin$Test))

PatObsGoldClin<-select(PatObsGoldClin, -readcode, term=desc, -adid, -patdate, -enttype)
PatObsAurum<-select(PatObsAurum, -value, -numunitid)

PatRayAll<-rbind(PatObsAurum, PatObsGoldClin)

save(PatRayAll, file=paste0(ProjectDir, "/Extracts/RayAll.Rdata"))
